<template>
  <div class="p-6 max-w-2xl mx-auto">
    ...
    <form @submit.prevent="submit" class="mb-8 flex gap-3">
      <input v-model="form.name" type="text" placeholder="Enter municipality name..."
    ...
    <ul v-if="municipalities.length" class="space-y-4">
      <li v-for="muni in municipalities" :key="muni.id"
        ...
        <button @click="deleteMunicipality(muni.id)"
  ...
  <div class="flex justify-center flex-col items-center gap-8">
    <LeafletComponent />
    <ProjectTimeline class="w-full max-w-4xl" />
  </div>
</template>
